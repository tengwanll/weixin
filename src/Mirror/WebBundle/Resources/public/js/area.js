...
$(function(){
	...
	$(document).on('click', '.address-shen', function(event) { //out-page选择省出现
		event.preventDefault();
		pageIn($(".out-address-shen"));
	}).on('click', '.address-shi', function(event) { //out-page选择市出现
		event.preventDefault();
		...
		pageIn($(".out-address-shi"));
	}).on('click', '.address-xian', function(event) { //out-page选择县出现
		event.preventDefault();
		var province = $(".address-shen").find(".address-content").text();
		...
		pageIn($(".out-address-xian"));
	}).on('click', '.out-header', function(event) { //点击头部关闭out-page
		event.preventDefault();
		pageOut($(this).closest('.out-page'));
	}).on('click', '.out-address-list li', function(event) { //选择选项
		event.preventDefault();
		var page=$(this).closest('.out-page');
		var target=page.attr('data-btn');
		var txt=$(this).text();
		if(target=="address-shen"){
			var id = $(this).attr("id");
			var shi_ul = $("#shi-list");
			shi_ul.find("li").remove();
			sar=dsy.Items["0_"+id];
			for(i=0;i<sar.length;i++){
				var li = document.createElement("li");
				li.setAttribute("id","0_"+id+"_"+i);
				li.innerHTML = sar[i];
				shi_ul.append(li);
			}
			$(".address-shi").find('.address-content').text("请选择");
			$(".address-xian").find('.address-content').text("请选择");
		}
		if(target=="address-shi"){
			var id = $(this).attr("id");
			var xian_ul = $("#xian-list");
			xian_ul.find("li").remove();
			sar=dsy.Items[id];
			for(i=0;i<sar.length;i++){
				var li = document.createElement("li");
				li.innerHTML = sar[i];
		...
		$("."+target).find('.address-content').text(txt);
